	...
	function handleFile(e) {
		...
		for (var i = 0; i<files.length; i++) {
			...
			filestring = "<strong>" + escape(f.name) + 
							":</strong> " + f.size + " bytes - " + 
							"<strong>Modificado el día:</strong> " + (f.lastModifiedDate ? f.lastModifiedDate.toLocaleDateString() : "n/a");
			...
			filereader.onload = (function(theFile) {
				return function(e) {
					if (outputdiv.substr(1,3) === "xml") {
						...
						client.open('GET', 'http://localhost:4000/stamp/resources/cfdv3.xsd');
						client.onreadystatechange = function() {
  							schemaData = client.responseText;
  							schemaFileName = 'cfdv3.xsd';
						}
						client.send();
		...
		$(outputdiv).html(filestring).css("background", "#ff9479").css("border", "2px solid #fd5236");
	}
